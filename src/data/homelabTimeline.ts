...
export const homelabTimeline: TimelineEntry[] = [
  ...
  },
  {
    ...
    body: "With the workflow established, an e-waste-bound machine was rebuilt on Debian 13 (Trixie): a single-core AMD Sempron 2800+ at 1.6 GHz with ~1.4 GiB of RAM, running a modern 6.12 kernel on a two-decade-old ASRock board. Hardened and tunneled, it became a personal cloud reachable only via SSH forwarding — proof that durable hardware and a lean, secure OS still hold up.",
    image: "sempron-specs.png",
    caption: "server-sempron — a single-core AMD Sempron 2800+ still running Debian 13.",
    filled: true,
  },
  {
    year: "2026",
    ...
    body: "The Dell OptiPlex 380, now on Ubuntu Server, grew from a plain NAS into a full self-hosted cloud: Jellyfin on its own subdomain — paired with yt-dlp for a lossless FLAC library — replaced a streaming subscription, sitting alongside the rest of the service stack and reachable from anywhere through Cloudflare. A 2011 MacBook Pro on Arch Linux joined as the high-agility node, running a PostgreSQL and Redis stack in Docker Compose for a collaborative database project.",
    bodyHtml: "The Dell OptiPlex 380, now on Ubuntu Server, grew from a plain NAS into a full self-hosted cloud: Jellyfin on its own subdomain — paired with yt-dlp for a lossless FLAC library — replaced a streaming subscription, sitting alongside the rest of the service stack and reachable from anywhere through Cloudflare. A <button type=\"button\" class=\"cf-link\" data-drawer-open=\"mbp\" aria-haspopup=\"dialog\">2011 MacBook Pro on Arch Linux</button> joined as the high-agility node, running a PostgreSQL and Redis stack in Docker Compose for a collaborative database project.",
